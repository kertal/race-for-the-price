...
// --- Convert current video to GIF or MOV via ffmpeg.wasm ---
...
if (convertSelect) {
  ...
  convertSelect.addEventListener('change', () => {
    ...
    const tmpl = document.getElementById('tmpl-export-overlay');
    const overlay = tmpl.content.cloneNode(true).firstElementChild;
    overlay.querySelector('h3').textContent = 'Converting to ' + format.toUpperCase();
    const convertCanvas = overlay.querySelector('.export-canvas');
    if (convertCanvas) convertCanvas.style.display = 'none';
    document.body.appendChild(overlay);

    const convertStatusEl = overlay.querySelector('.export-status');
    const convertProgressFill = overlay.querySelector('.export-progress-fill');
    const convertActionsEl = overlay.querySelector('.export-actions');
    let convertCancelled = false;

    convertStatusEl.textContent = 'Fetching video...';
    overlay.querySelector('.export-cancel').addEventListener('click', () => {
      convertCancelled = true;
      overlay.remove();
    });

    const srcName = activeVideo.src.split('/').pop().replace(/\.webm$/i, '') || 'race-video';

    let clipRange = null;
    if (activeClip) {
      const adj = getAdjustedClipTimes();
      const ct2 = adj || clipTimes;
      const pIdx = raceVideos.indexOf(primary);
      if (pIdx >= 0 && ct2 && ct2[pIdx] && ct2[pIdx].start < ct2[pIdx].end) {
        clipRange = { start: ct2[pIdx].start, end: ct2[pIdx].end };
      }
    }

    fetch(activeVideo.src).then(response => {
