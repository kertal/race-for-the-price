...
export function copyFFmpegFiles(destDir) {
  ...
  try {
    ...
    // Resolve an exported entry point and navigate to the dist/esm directory
    ...
    for (const file of FFMPEG_ESM_FILES) {
      fs.copyFileSync(path.join(ffmpegEsmDir, file), path.join(ffmpegDir, file));
    }
    for (const file of FFMPEG_CORE_FILES) {
      fs.copyFileSync(path.join(coreEsmDir, file), path.join(ffmpegDir, file));
    }
    return true;
  } catch (e) {
